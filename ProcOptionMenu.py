...
class ProcOptionMenu:
    def __init__(self, master, database: ActionDataBase):
        self.database = database

        self.optionmenu = CTk.CTkOptionMenu(master=master,
                                            values=database.get_proc_names(),
                                            command=self.show_names)
        self.optionmenu.grid(row=2, column=7, columnspan=1)

        self.scrollbar_frame = CTk.CTkScrollableFrame(master=master, width=300)
        self.scrollbar_frame.grid(row=2, column=8, columnspan=2)

        self.labels = []
        self.entries = []
        self.button = None

    def show_names(self, proc_name: str) -> None:
        self.clear_lists()
        for i, name in enumerate(self.database.get_input_information(proc_name)):
            label = CTk.CTkLabel(master=self.scrollbar_frame, text=name)
            label.grid(row=i, column=0, sticky="nsew")
            self.labels.append(label)
        self.show_entry(proc_name)

    def show_entry(self, proc_name: str) -> None:
        for i, name in enumerate(self.database.get_input_information(proc_name)):
    ...
    def use(self) -> None:
        ...
        self.database.activate_procedure(proc_name, values)
